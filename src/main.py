...
def get_rvc_model(voice_model, is_webui):
    ...
    if rvc_model_filename is None:
        error_msg = f'No existe ningún archivo de modelo en {model_dir}.'
...
def preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress=None):
    keep_orig = False
    if input_type == 'yt':
        display_progress('[~] Descargando canción...', 0, is_webui, progress)
    ...
    display_progress('[~] Separando vocales del instrumental...', 0.1, is_webui, progress)
    ...
    display_progress('[~] Separando vocales principales de las vocales de fondo...', 0.2, is_webui, progress)
    backup_vocals_path, main_vocals_path = run_mdx(mdx_model_params, song_output_dir, os.path.join(mdxnet_models_dir, 'UVR_MDXNET_KARA_2.onnx'), vocals_path, suffix='Backup', invert_suffix='Main', denoise=True)

    display_progress('[~] Aplicando DeReverb a los vocales...', 0.3, is_webui, progress)
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        if not song_input or not voice_model:
            raise_exception('Asegurate que la canción y el nombre del modelo de voz estén llenos.', is_webui)

        display_progress('[~] Iniciando proceso de AI Cover...', 0, is_webui, progress)
        ...
        if urlparse(song_input).scheme == 'https':
            ...
            if song_id is None:
                error_msg = 'Enlace de YT inválido.'
                raise_exception(error_msg, is_webui)
        ...
        # local audio file
            ...
            if os.path.exists(song_input):
                ...
                error_msg = f'{song_input} no existe.'
                song_id = None
                raise_exception(error_msg, is_webui)
        ...
        if not os.path.exists(ai_vocals_path):
            display_progress('[~] Convirtiendo voz usando RVC...', 0.5, is_webui, progress)
        ...
        display_progress('[~] Aplicando efectos de audio a los vocales...', 0.8, is_webui, progress)
        ai_vocals_mixed_path = add_audio_effects(ai_vocals_path, reverb_rm_size, reverb_wet, reverb_dry, reverb_damping)

        if pitch_change_all != 0:
            display_progress('[~] Aplicando el cambio de tono', 0.85, is_webui, progress)
            instrumentals_path = pitch_shift(instrumentals_path, pitch_change_all)
            backup_vocals_path = pitch_shift(backup_vocals_path, pitch_change_all)

        display_progress('[~] Combinando las vocales IA con el instrumental...', 0.9, is_webui, progress)
        ...
        if not keep_files:
            display_progress('[~] Eliminando archivos de audio innecesarios...', 0.95, is_webui, progress)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Genera un AI Cover en el directorio song_output/id.', add_help=True)
    ...
    if not os.path.exists(os.path.join(rvc_models_dir, rvc_dirname)):
        raise Exception(f'El directorio {os.path.join(rvc_models_dir, rvc_dirname)} no existe.')
    ...
    print(f'[+] Cover generado en: {cover_path}')
